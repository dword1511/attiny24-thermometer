...
uint8_t DS18X20_start_meas(uint8_t with_power_extern, uint8_t id[]) {
  ...
  if(ow_input_pin_state()) { /* only send if bus is "idle" = high */
    if(with_power_extern != DS18X20_POWER_EXTERN) ow_command_with_parasite_enable(DS18X20_CONVERT_T, id); /* not longer needed: ow_parasite_enable(); */
    else ow_command(DS18X20_CONVERT_T, id);
    ret = DS18X20_OK;
  }
  else ret = DS18X20_START_FAIL;
  return ret;
}
...
uint8_t DS18X20_read_scratchpad(uint8_t id[], uint8_t sp[], uint8_t n) {
  ...
  ow_reset();
  if(ow_input_pin_state()) ret = read_scratchpad(id, sp, n); /* only send if bus is "idle" = high */ 
  else ret = DS18X20_ERROR;
...
uint8_t DS18X20_scratchpad_to_eeprom(uint8_t with_power_extern, uint8_t id[]) {
  ...
  ow_reset();
  if(ow_input_pin_state()) { /* only send if bus is "idle" = high */
    if(with_power_extern != DS18X20_POWER_EXTERN) ow_command_with_parasite_enable(DS18X20_COPY_SCRATCHPAD, id); /* not longer needed: ow_parasite_enable(); */
    else ow_command(DS18X20_COPY_SCRATCHPAD, id);
    _delay_ms(DS18X20_COPYSP_DELAY); /* wait for 10 ms */
    if(with_power_extern != DS18X20_POWER_EXTERN) ow_parasite_disable();
    ret = DS18X20_OK;
  } 
  else ret = DS18X20_START_FAIL;
