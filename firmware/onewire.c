...
uint8_t ow_byte_wr_with_parasite_enable(uint8_t b) {
  ...
  do {
    if (i != 1) j = ow_bit_io_intern(b & 1, 0);
    else j = ow_bit_io_intern(b & 1, 1);
    b >>= 1;
    if(j) b |= 0x80;
  } while(--i);
...
uint8_t ow_rom_search(uint8_t diff, uint8_t *id) {
  ...
  if(ow_reset()) return OW_PRESENCE_ERR; /* error, no device found <--- early exit! */
  ...
  do {
    ...
    do {
      ...
      if(ow_bit_io(1)) { /* use brackets to tell the compiler the following else is mine */
...
static void ow_command_intern(uint8_t command, uint8_t *id, uint8_t with_parasite_enable) {
  ...
  if(with_parasite_enable) ow_byte_wr_with_parasite_enable(command);
  else ow_byte_wr(command);
}
